...
const Slider = () => {
	...
	const handleDelete = async (agencyId) => {
	  if (window.confirm('Are you sure you want to delete this slider?')) {
		try {
		  const response = await axios.delete(`http://localhost:8000/admin/sliders/${agencyId}`);
		  if (response.data.success) {
                    ...
                    <tbody>
                      {agencies.map((agency, index) => (
                        <tr key={agency._id}>
                          ...
                          <td> <img src={`http://localhost:8000/${agency.sliderImage}`} alt="" width={30} height={30} /> </td>
                          <td>
